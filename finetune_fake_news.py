...
def main():
    ...
    tokenized_datasets = dataset.map(lambda x: tokenizer(x['text'], padding='max_length', truncation=True, max_length=max_length), batched=True)
    tokenized_datasets = tokenized_datasets.remove_columns(['text', '__index_level_0__']) if '__index_level_0__' in tokenized_datasets['train'].features else tokenized_datasets.remove_columns(['text'])
